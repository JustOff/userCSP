...
var userCSP = require("sdk/panel").Panel({
  width: 800,
  height: 600,
...
require("sdk/widget").Widget({
  ...
  contentURL: data.url("CSP_logo-48_54.jpg"),
  panel: userCSP
});
       ...
       //  console.log("\n responseName = " + responseName);
        ...
        try {
            ...
            try {
                ...
                try {
                    cspRules = httpChannel.getResponseHeader("X-Content-Security-Policy-Report-Only"); // report-only mechanims support 
                    websiteHeaderState = 3;
                } catch (e) { // catch 3
                    try {
                        cspRules = httpChannel.getResponseHeader("Content-Security-Policy-Report-Only"); // report-only mechanims support 
                ...
                if (typeof(ss.storage.userCSPPoliciesArray[responseName]) !== "undefined") {
                    if (ss.storage.userCSPPoliciesArray[responseName][11] !== null || ss.storage.userCSPPoliciesArray[responseName][11] !== "") {
                        if (websiteHeaderState === 0 || websiteHeaderState === 1) {
                            ...
                            httpChannel.setResponseHeader("X-Content-Security-Policy-Report-Only", ss.storage.userCSPPoliciesArray[responseName][11], false);
                        } else if (websiteHeaderState === 4) {
                            httpChannel.setResponseHeader("Content-Security-Policy-Report-Only", ss.storage.userCSPPoliciesArray[responseName][11], false);
                        }
